...
postDiv.addEventListener('click', async () => {
  ...
  const cssLink = document.querySelector('link[href="/post_static/vel-postform.css"]');
  if (!cssLink) {
    const newCssLink = document.createElement('link');
    newCssLink.rel = 'stylesheet';
    newCssLink.href = '/post_static/vel-postform.css';
    document.head.appendChild(newCssLink);}

  const overlayDiv = document.querySelector('.overlay');
  // add and load script
  const script = document.createElement('script');
  script.src = '/post_static/activate-postform.js'; // replace with the path to your script file
